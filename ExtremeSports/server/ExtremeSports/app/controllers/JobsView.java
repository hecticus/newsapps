...
public class JobsView extends HecticusController {
    ...
    public static Result blank() {
        ...
        return ok(form.render(JobViewForm));
    }

    @Restrict(@Group(Application.ADMIN_ROLE))
    public static Result list(int page, String sortBy, String order, String filter) {
        return ok(list.render(Job.page(page, 10, sortBy, order, filter), sortBy, order, filter, false));
    }
    ...
    public static Result update(Long id) {
        Form<Job> filledForm = JobViewForm.bindFromRequest();
        if(filledForm.hasErrors()) {
            System.out.println(filledForm.toString());
            return badRequest(edit.render(id, filledForm));
        }
        Job gfilledForm = filledForm.get();
        String date = filledForm.data().get("Date");//Date=01/01/2014 06:00
        if(!gfilledForm.isDaemon()) {
            switch (gfilledForm.getFrequency()) {
                case 0://once
                    gfilledForm.setTimeParams(date.substring(6, 10) + date.substring(3, 5) + date.substring(0, 2));
                    gfilledForm.setTime(date.substring(11).replace(":", ""));
                    break;
                case 1://year
                    gfilledForm.setTimeParams(date.substring(3, 5) + date.substring(0, 2));
                    gfilledForm.setTime(date.substring(11).replace(":", ""));
                    break;
                case 2://month
                    gfilledForm.setTimeParams(date.substring(0, 2));
                    gfilledForm.setTime(date.substring(11).replace(":", ""));
                    break;
                case 3://week
                    int time = 0;
                    if (filledForm.data().containsKey("sunday")) {
                        time += Math.pow(2, 0);
                    }

                    if (filledForm.data().containsKey("monday")) {
                        time += Math.pow(2, 1);
                    }

                    if (filledForm.data().containsKey("tuesday")) {
                        time += Math.pow(2, 2);
                    }

                    if (filledForm.data().containsKey("wednesday")) {
                        time += Math.pow(2, 3);
                    }

                    if (filledForm.data().containsKey("thursday")) {
                        time += Math.pow(2, 4);
                    }

                    if (filledForm.data().containsKey("friday")) {
                        time += Math.pow(2, 5);
                    }

                    if (filledForm.data().containsKey("saturday")) {
                        time += Math.pow(2, 6);
                    }
                    gfilledForm.setTimeParams("" + time);
                    gfilledForm.setTime(date.substring(11).replace(":", ""));
                    break;
                case 4://day
                    gfilledForm.setTimeParams("");
                    gfilledForm.setTime(date.substring(11).replace(":", ""));
                    break;
            }
        } else {
            gfilledForm.setTimeParams(filledForm.data().get("daemonFrequency"));
        }

        gfilledForm.update(id);
        flash("success", Messages.get("jobs.java.updated", gfilledForm.getName()));
        return GO_HOME;
    ...
    public static Result lsort() {
        return ok(list.render(Job.page(0, 0, "configKey", "asc", ""),"date", "asc", "",true));
    }
    ...
    public static Result submit() throws IOException {
        ...
        gfilledForm.save();
        flash("success", Messages.get("jobs.java.created", gfilledForm.getName()));
        return GO_HOME;
