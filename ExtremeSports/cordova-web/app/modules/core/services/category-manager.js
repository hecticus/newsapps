...
CategoryManager.$inject = ['$http', '$q', 'Domain'];
function CategoryManager($http, $q, Domain) {
    ...
    function get(){
        ...
        return $http.get(Domain.categories.list(), config).then(success, error);

        function success(response){
            if(response.data.error === 0){
                return response.data.response;
            } else{
                console.log('Error retrieving categories from server on success callback: '
        ...
        function error(response){
            console.log('Error retrieving categories from server: ');
            console.log(response.data);
            return $q.reject(response);
        }
    }
