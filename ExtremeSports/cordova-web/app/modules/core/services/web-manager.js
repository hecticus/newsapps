 ...
 */
angular.module('core').factory('WebManager', WebManager);

WebManager.$inject = ['$http', 'CordovaDevice', 'AthleteManager', 'CategoryManager', 'Domain', 'Client', 'App'];

function WebManager($http, CordovaDevice, AthleteManager, CategoryManager, Domain, Client, App) {
    ...
    function getFavoritesConfig(isFilterActive){
        ...
        if(isFilterActive){
            httpConfig.params.athletes = AthleteManager.getFavorites().map(function(elem){
                return elem.id_athlete;
            });
            console.log('httpConfig.params.athlete: ');
            console.log(httpConfig.params.athletes);

            httpConfig.params.categories = CategoryManager.getFavorites().map(function(elem){
                return elem.id_category;
            });
            console.log('httpConfig.params.categories: ');
            console.log(httpConfig.params.categories);
        }
        return httpConfig;
    }
