CREATE TABLE `leaderboard_push` (
  `id_leaderboard_push` int(11) NOT NULL AUTO_INCREMENT,
  `id_client` int(11) DEFAULT NULL,
  `score` int(11) DEFAULT NULL,
  PRIMARY KEY (`id_leaderboard_push`),
  UNIQUE KEY `uq_client_score_push` (`id_client`),
  KEY `fk_lb_clients_score_push_idx` (`id_client`),
  CONSTRAINT `fk_lb_clients_points_push` FOREIGN KEY (`id_client`) REFERENCES `clients` (`id_client`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

ALTER TABLE `football_brazil`.`leaderboard` 
ADD UNIQUE INDEX `uq_client_tournament_phase` (`id_client` ASC, `id_tournament` ASC, `id_phase` ASC);

ALTER TABLE `football_brazil`.`leaderboard_global` 
ADD UNIQUE INDEX `uq_client_tournament` (`id_client` ASC, `id_tournament` ASC);
...
#se usa para filtrar las apuestas a calcular, 1 para no calculada y 3 para calculada). Una apuesta se hace seteando el campo client_bet (1 si gana home, 2 si empatan y 3 si gana away)
